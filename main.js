...
window.addEventListener('load', function () {
   ...
   //Нажатие на иконку корзины
    ...
    iconCart.addEventListener('click',() => cart.basket.style.display = 'block');

    //Нажатие на иконку закрыть корзину
    const iconClose = document.querySelector('.drop__close');
    iconClose.addEventListener('click',(evt) => {
        cart.basket.style.display = 'none';
        evt.stopPropagation();
    });
})
